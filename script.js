...
function initCustomCursor() {
  ...
  const hoverSelectors = 'a, button, .neo-btn, .sticker, input, textarea, select, .project-filter-btn, .nav-logo, canvas';
...
function initPageTransitions() {
  ...
  // Dismiss the overlay that covers screen on page entry.
  let dismissed = false;
  function dismissOverlay() {
    ...
    setTimeout(() => {
      // After animation completes, move fully out and disable pointer events
      overlay.style.transform = 'translateY(-100%)';
      overlay.style.pointerEvents = 'none';
    }, 600);
  }

  // Dismiss on DOM ready or window load or immediately if already ready
  if (document.readyState !== 'loading') {
    requestAnimationFrame(() => setTimeout(dismissOverlay, 50));
  } else {
    document.addEventListener('DOMContentLoaded', () => setTimeout(dismissOverlay, 50), { once: true });
    window.addEventListener('load', dismissOverlay, { once: true });
  }

  // Safety fallback: ensure overlay is dismissed within 700ms under any circumstances
  ...
  links.forEach(link => {
    ...
    if (isLocal && !isTargetBlank) {
      link.addEventListener('click', (e) => {
        ...
        // Reset to bottom, then slide up to cover screen
        overlay.style.transform = 'translateY(100%)';
